...
public class MainMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonNetwork.setOnClickListener(v -> {
            Intent intent = new Intent(MainMainActivity.this, NetworkActivity.class);
            startActivity(intent);
        });

        Button buttonBluetooth = findViewById(R.id.useBluetooth);
        buttonBluetooth.setOnClickListener(v -> {
            Intent intent = new Intent(MainMainActivity.this, BluetoothActivity.class);
            startActivity(intent);
        });
        ...
        buttonSupport.setOnClickListener(v -> {
            ...
            if (rewardedAd != null) {
                rewardedAd.show(MainMainActivity.this, rewardItem -> {
                    Toast.makeText(MainMainActivity.this, "Thank you for your support!", Toast.LENGTH_SHORT).show();
                    GlobalSettings.getInstance().setRewardedAd(null);
                    loadRewardedAd();
                });
            } else {
                Toast.makeText(MainMainActivity.this, "Ad is not ready yet, please press the button again.", Toast.LENGTH_SHORT).show();
                loadRewardedAd();
            }
        });
    }
    ...
    private void loadRewardedAd() {
        if(GlobalSettings.getInstance().isLoadingAd()){
            Toast.makeText(MainMainActivity.this, "Ad is still loading,press the button again after a while.", Toast.LENGTH_SHORT).show();
            return;
        }
